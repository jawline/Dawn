...
typedef unsigned long (*io_operation) (struct filesystem_node* node, unsigned long offset, unsigned long size, uint8_t* ptr);

typedef void (*cmd_operation) (struct filesystem_node* node);

typedef struct dirent (*read_directory_t) (struct filesystem_node* node, uint32_t idx);
...
struct filesystem_node {
	...
	uint32_t flags; //32 bit bitmask for flags
