...
static uint16_t* video_memory_location = (uint16_t*) (KERNEL_START + 0xB8000);
...
static const uint8_t active_foreground_col = 15;
static const uint8_t active_background_col = 0;
...
void text_mode_scroll_prec(uint16_t* video_memory, unsigned char bgc, unsigned char fgc, unsigned int* cx, unsigned int* cy)
{
    // Get a space character with the default colour attributes.
    uint8_t attributeByte = (bgc << 4) | (fgc & 0x0F);
...
void text_mode_putc_prec(uint16_t* video_memory, uint8_t bgc, uint8_t fgc, unsigned int* cx, unsigned int* cy, char c)
{
...
void text_mode_clearscreen_prec(uint16_t* video_memory, uint8_t bgc, uint8_t fgc, unsigned int* cx, unsigned int* cy)
{
    // Make an attribute byte for the default colours
    uint8_t attributeByte = (bgc /*black*/ << 4) | (fgc /*white*/ & 0x0F);
    uint16_t blank = 0x20 /* space */ | (attributeByte << 8);
...
void text_mode_set_bg_color(uint8_t col)
{
}
...
{
	memcpy(term->m_backupData, video_memory_location, sizeof(uint16_t) * term->m_width * term->m_height);
}
...
void text_mode_tsetfg(terminal_t* term, uint8_t col)
{
	text_mode_set_fg_color(col);
}

void text_mode_tsetbg(terminal_t* term, uint8_t col)
{
...
void text_mode_sett(terminal_t* term)
{
	...
	memcpy(video_memory_location, term->m_backupData, sizeof(uint16_t) * term->m_width * term->m_height);
