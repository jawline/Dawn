...
{
	uint8 type = detect_device_type(dev);
...
uint8 device_read(uint16* dest, uint32 offset_sectors, uint32 num_sectors, disk_device device)
{
	...
	for (;;)
	{
		...
		} else if (num_sectors > 256)
		{
		  ...
		  dest = ((uint32*)dest) + (sizeof(uint16) * 256); //Increment the destination pointer by that
...
uint8 device_write(uint16* source, uint32 offset_sectors, uint32 num_sectors, disk_device device)
{
