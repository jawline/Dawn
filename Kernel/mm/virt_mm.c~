...
uint32 copy_page_table(uint32 * pt, uint32 copy)
{
	...
	for (iter = 0; iter < 1024; iter++)
	{
		if (existing_addr[iter] != 0)
		{
			...
			copy_frame(existing_addr[iter], copied_p_addr);
...
page_directory_t* copy_page_dir(page_directory_t* pagedir) 
{
	...
	for (iter = KERNEL_TABLES; iter < 1023; iter++)
	{
		...
		{
			//Copy the page table
			temp_newdir_map[iter] = copy_page_table(c_addr[iter], 1) | PAGE_PRESENT | PAGE_WRITE;
		}
	}
	...
	map(firstfree, temp_newdir_map[1022] >> 12, PAGE_PRESENT | PAGE_WRITE);
